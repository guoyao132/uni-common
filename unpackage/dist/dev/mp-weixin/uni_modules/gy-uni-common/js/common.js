...
const gyCommon = {
  ...
  */
  watermark({
    ...
    id,
    defaultSettings = {
      watermark_color: "#0e8eb5",
      //水印字体颜色
      watermark_alpha: 0.1,
      //水印透明度
      watermark_fontsize: "17px",
      //水印字体大小
      watermark_font: "微软雅黑",
      watermark_width: 200,
      //水印宽度
    ...
    let rowNumber = (maxWidth - 40 - 200) / intervalWidth;
    let coumnNumber = (maxHeight - 40 - 150) / intervalheight;
    ...
    if (isOne) {
      ...
      for (let i2 = 0; i2 < rowNumber; i2++) {
        for (let j2 = 0; j2 < coumnNumber; j2++) {
          let x = intervalWidth * i2 + 20;
          let y = intervalheight * j2 + 30;
          addMarkFun(x, y);
        }
      }
    }
  ...
  */
  getNameById(val, list, idCol, nameCol) {
    if (list) {
      for (let i2 in list) {
        if (list[i2][idCol] == val)
          return list[i2][nameCol];
      }
    }
    return null;
  },
  ...
  formatTime: function(time, type, customFormat) {
    ...
    let dataStr = "";
    let y = date.getFullYear().toString(), m = date.getMonth() < 9 ? "0" + (date.getMonth() + 1) : date.getMonth() + 1, d = date.getDate() < 10 ? "0" + date.getDate() : date.getDate(), h = date.getHours() < 10 ? "0" + date.getHours() : date.getHours(), M = date.getMinutes() < 10 ? "0" + date.getMinutes() : date.getMinutes(), s = date.getSeconds() < 10 ? "0" + date.getSeconds() : date.getSeconds();
    dataStr = y + "-" + m + "-" + d + " " + h + ":" + M + ":" + s;
    if (customFormat) {
  ...
  */
  timeDifference(date, date2, timeDiff, geshi) {
    ...
    if (date2) {
      ...
      jieshu = /* @__PURE__ */ new Date();
    }
    if (!timeDiff) {
      timeDiff = 0;
    }
    let dateDiff = jieshu.getTime() - kaishi.getTime() - timeDiff;
        ...
        if (obj === null) {
          ...
          if (Object.prototype.toString.call(obj).slice(8, -1) === "Array") {
            ...
            for (let i2 = 0; i2 < obj.length; i2++) {
              copy.push(this.cloneObj(obj[i2]));
            }
          } else {
            copy = {};
            for (let j2 in obj) {
              copy[j2] = this.cloneObj(obj[j2]);
            }
          }
        }
        break;
  ...
  getEvryDay: function(date) {
    let curDate = date ? new Date(date) : /* @__PURE__ */ new Date();
  ...
  tree2array: function(tree, child) {
    let lists = [];
    let toArr = function(children, child2) {
      for (let i2 = 0; i2 < children.length; i2++) {
        let c = children[i2][child2];
        lists.push(children[i2]);
        if (c && c.length > 0) {
          toArr(c, child2);
        }
      }
